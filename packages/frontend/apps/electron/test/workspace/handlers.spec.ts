...
import { universalId } from '@madisboard/nbstore';
...
vi.doMock('@madisboard/electron/helper/db/ensure-db', () => ({
  ensureSQLiteDB: async () => ({
    destroy: () => {},
  }),
}));

vi.doMock('@madisboard/electron/helper/main-rpc', () => ({
  mainRPC: {
...
afterAll(() => {
  vi.doUnmock('@madisboard/electron/helper/main-rpc');
});

describe('workspace db management', () => {
  test('trash workspace', async () => {
    const { trashWorkspace } =
      await import('@madisboard/electron/helper/workspace/handlers');
